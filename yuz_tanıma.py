#face_recognition ve opencv kütüphanelerini import ederek başlıyoruz 
import face_recognition
import cv2
import numpy as np

#opencv metodu olan VideoCapture ile webcam'den görüntü almayı başlatıyoruz 
video_capture = cv2.VideoCapture(0)

#"kişi1'in" resmini yüklüyoruz ve encoding bilgisini alıyoruz. 
kişi1_image = face_recognition.load_image_file("kişi1.jpg")
kişi1_face_encoding = face_recognition.face_encodings(kişi1_image)[0]
# "kişi2'nin" resmini yüklüyoruz ve encoding bilgisini alıyoruz
kişi2_image = face_recognition.load_image_file("kişi2.jpg")
kişi2_face_encoding = face_recognition.face_encodings(kişi2_image)[0]

# Encoding ve açıklama kısmını burada tanımlıyoruz, birden fazla tanımlayabiliriz
known_face_encodings = [
  
    kişi1_face_encoding,
    kişi2_face_encoding,

]
known_face_names = [

    "kişi1",
    "kişi2",
]

face_locations = []
face_encodings = []
face_names = []
process_this_frame = True

while True:
   # Videodan anlık bir kare yakalıyoruz
    ret, frame = video_capture.read()

    
    if process_this_frame:

         # Aldığımız kareyi 1/4 oranında küçültüyoruz ve bu daha hızlı sonuç vermeyi sağlıyor
        small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)

        # BGR(opencv) türündeki resmi RGB(face_recognition) formatına çeviriyoruz
        rgb_small_frame = small_frame[:, :, ::-1]

        # Uyumlu tüm yüzlerin lokasyonlarını bulan kodlar
        face_locations = face_recognition.face_locations(rgb_small_frame)
        face_encodings = face_recognition.face_encodings(rgb_small_frame, face_locations)

        face_names = []
         # Eşleşen yüzleri topla
        for face_encoding in face_encodings:
           
            matches = face_recognition.compare_faces(known_face_encodings, face_encoding)
            name = "Unknown"

            face_distances = face_recognition.face_distance(known_face_encodings, face_encoding)
            best_match_index = np.argmin(face_distances)
            # Bilinen face_encodings içinde bir eşleşme bulunursa, sadece ilkini kullanın.
            if matches[best_match_index]:
                name = known_face_names[best_match_index]

            face_names.append(name)

    process_this_frame = not process_this_frame
  # Sonuçları göster
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        top *= 4
        right *= 4
        bottom *= 4
        left *= 4

         # Yüzü çerçeve içerisine al
        cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)

        cv2.rectangle(frame, (left, bottom - 35), (right, bottom), (0, 0, 255), cv2.FILLED)
        font = cv2.FONT_HERSHEY_DUPLEX
        cv2.putText(frame, name, (left + 6, bottom - 6), font, 1.0, (255, 255, 255), 1)

  # Oluşan çerçeveyi ekrana yansıt
    cv2.imshow('Video', frame)
# Çıkış için 'q'
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break
# Kamerayı kapat
video_capture.release()
cv2.destroyAllWindows()